...
namespace dnpsoup {
  ...
  MatrixCxDbl t1SuperOp(
      double t1_inv,
      //[[maybe_unused]] const MatrixCxDbl &eigen_vec,
      const MatrixCxDbl &x, 
      const MatrixCxDbl &y)
  {
    ...
    auto part_x = secularRelaxationSuperOp(x);
    auto part_y = secularRelaxationSuperOp(y);
      ...
      const MatrixCxDbl &z)
  {
    ...
    return t2_inv * secularRelaxationSuperOp(z);
  }
} // namespace dnpsoup
